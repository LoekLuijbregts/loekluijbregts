...
const planner = "https://zcal.co/i/vcLERywI?via=improve-your-scheduling-tool";
...
const common = {
  nl: {
    ...
    source: "Bronvermelding", affiliate: "Affiliatevermelding",
  ...
  en: {
    ...
    source: "Source reference", affiliate: "Affiliate disclosure",
...
const toolBase = {
  vidonary: {
    name: "Vidonary", accent: "coral", affiliate: "https://get.vidonary.com/q6u2niq0p6qt",
    offers: [{ url: "https://get.vidonary.com/q6u2niq0p6qt" }],
    process: ["Idea", "Script", "Record", "Edit", "Publish"],
  },
  manychat: {
    name: "ManyChat", accent: "violet", affiliate: "https://manychat.partnerlinks.io/mzf2x66x80z0-ogcg6e",
  ...
  scoreapp: {
    name: "ScoreApp", accent: "blue", affiliate: "https://share.scoreapp.com/89af2cbb",
    offers: [{ url: "https://share.scoreapp.com/89af2cbb" }], image: "/assets/tools/scoreapp-why.webp", imageAlt: "ScoreApp scorecard workflow",
  },
  zcal: {
    name: "zcal", accent: "gold", affiliate: "https://zcal.co?via=improve-your-scheduling-tool",
  ...
  phantombuster: {
    name: "PhantomBuster", accent: "periwinkle", affiliate: "https://phantombuster.com?deal=loek15",
...
const copy = {
  nl: {
    vidonary: {
      ...
      cases: [
        ["time","workflow","60 sec","Van lege pagina naar een bruikbaar videoplan","Vidonary zegt dat een eerste videoplan in 60 seconden kan staan. Script, teleprompter, montage en publicatie blijven daarna in dezelfde werkwijze.","Vidonary productwerkwijze"],
        ["money","benchmark","$10M+","Herhaalbare video als bron van commerciële vraag","Vidonary meldt dat het oprichtersteam meer dan 50 miljoen views en ruim 10 miljoen dollar omzet met video realiseerde. De tool vertaalt die ervaring naar een eenvoudiger proces voor kleine teams.","Vidonary teamresultaten"]
      ],
    },
    manychat: {
      ...
      cases: [
        ["time","story","15 uur p/m","Candace Junée stopte met elke Instagram-DM handmatig beantwoorden","ManyChat meldt dat trefwoorden en Story-triggers haar reactietijd met 99% verkortten, 15 uur per maand bespaarden en 118% meer leads opleverden.","ManyChat klantverhaal: Candace Junée"],
        ["money","story","$550K","Matze Brandmüller automatiseerde webinar en winkelwagenopvolging","ManyChat meldt ruim 550.000 dollar omzet in zes maanden, plus hogere webinarinschrijvingen en herstel van verlaten winkelwagens.","ManyChat klantverhaal: Chatbot Creator Bootcamp"]
      ],
    },
    scoreapp: {
      ...
      cases: [
        ["time","story","24/7","Nell Mead geeft patiënten direct bruikbare richting","ScoreApp beschrijft twee vragenlijsten die vrijwel direct gestructureerde begeleiding geven, onderzoek verzamelen en meer mensen helpen dan één behandelaar handmatig kan.","ScoreApp klantverhaal: Nell Mead"],
        ["money","story","$12M","Jason Graystone kwalificeerde intentie vóór opvolging","ScoreApp meldt 12 miljoen dollar in tien maanden zonder advertenties of salesteam, met 36% conversie uit een assessment-gedreven funnel.","ScoreApp klantverhaal: Jason Graystone"]
      ],
    ...
    zcal: {
      ...
      cases: [
        ["time","workflow","1 link","Vervang planningsmailtjes en losse herinneringen","Met zcal meeting polls stemmen meerdere gasten op beschikbare tijden. Kalenderafspraken, bevestigingen en herinneringen worden daarna automatisch geregeld.","zcal functies voor plannen en herinneren"],
        ["money","benchmark","+13%","Laat meer kalenderbezoekers een gesprek boeken","zcal meldt een 13% hogere boekingsratio dan het branchegemiddelde. Zo bereikt meer van de al opgebouwde interesse daadwerkelijk een gesprek.","zcal benchmark voor boekingsratio"]
      ],
    },
    phantombuster: {
...
function detailPage(lang,slug) {
  ...
  const cases=d.cases.map(x=>`<article><div class="public-case-meta"><span>${c.cases[x[0]]}</span><small>${c.cases[x[1]]}</small></div><strong>${x[2]}</strong><h3>${x[3]}</h3><p>${x[4]}</p><span class="source-link">${c.source}: ${x[5]}</span></article>`).join("");
...
await import("./check-affiliate-links.mjs");
